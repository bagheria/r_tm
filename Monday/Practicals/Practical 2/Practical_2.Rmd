...
Diana <- read.csv("Diana.csv", stringsAsFactors = FALSE)
Michael <- read.csv("Michael.csv",stringsAsFactors = FALSE)
```
...
```{r 25, echo=FALSE, warning = FALSE, include = TRUE}
...
```{r 26, echo=FALSE, warning = FALSE, include = TRUE}
wordcloud(Diana$content, scale = c(7,.13), min.freq = 50, colors = pal)
...
grafDianapos <-ggplot(DianastatsPOS, aes(key, freq_pct)) +geom_bar(stat = "identity", aes(fill = key), position = "dodge") + My_Theme + coord_flip() 
grafDianapos
```
...
grafDinoun <-ggplot(Distatsnoun, aes(x, freq)) + geom_bar(stat = "identity", aes(fill = x), position = "dodge") + My_Theme + coord_flip() 
grafDinoun
```
...
grafDiadj + geom_bar(stat = "identity", aes(fill = x), position = "dodge") + My_Theme + coord_flip() 
```
...
grafMichaelpos +geom_bar(stat = "identity", aes(fill = key), position = "dodge")  + My_Theme + coord_flip() 
```
...
grafMicadj + geom_bar(stat = "identity", aes(fill = x), position = "dodge") + My_Theme + coord_flip() 
```
