...
author: "Ayoub Bagheri & Qixiang Fang"
output:
...
- Term frequency (TF): the frequency count of term $t$ in doc $d$
...
$$C_{NB} = \underset{c \in C}{\operatorname{argmax}}P(w_1 | c) \cdot P(w_2|c) \cdot P(w_3|c) \cdot \ldots \cdot P(w_n|c) \cdot P(c)$$
...
$$\hat{P}(c) = \frac{ccount(C = c)}{N_{doc}}$$
...
- <b>Precision</b> (also Positive Predictive Value): % of selected items that are correct <br> <b>Recall</b>: % of correct items that are selected

- Precision is a valid choice of evaluation metric when we want to be very sure of our prediction.

- Recall (also sensitivity) is a valid choice of evaluation metric when we want to capture as many positives as possible.
...
where $\beta$ is a positive real factor and is chosen such that recall is considered $\beta$ times as important as precision. 
